  ...
  .controller('CropForecastCtrl', function ($rootScope, $scope, setup, tools,
      ...
      */
      function draw_forecast(cu) {
          ...
          var calendar = new Calendar(base_c, setup.getMonths(), setup.getDaysCalendar(), $scope.crop_vars, $scope.crop_yield_var.name, '#back_' + cu.id, '#forward_' + cu.id, '#current_month_' + cu.id, $scope.yield_ranges, 'c_alias_' + cu.id);
          calendar.render();
          ...
          for (var j = 0; j < $scope.crop_vars.length; j++) {
              ...
              $("#yield_" + vr.name + "_" + cu.id + "_max_date").html(summary_vr.max.date);
              $("#yield_" + vr.name + "_" + cu.id + "_max_yield").html(summary_vr.max.value.toFixed(float) + ' ' + vr.metric);
              $("#yield_" + vr.name + "_" + cu.id + "_min_date").html(summary_vr.min.date);
