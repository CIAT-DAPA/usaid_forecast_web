/**
  ...
  */
async function plot_map(id, ws, crops, country, countries) {
    if (country == "" || country == null) {
        await navigator.geolocation.getCurrentPosition((position) => {
            fetch('https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/reverseGeocode?f=json&location='+ position.coords.longitude.toString() + ',' + position.coords.latitude.toString())
                .then(response => response.json())
                .then(json => {
                    var response = validationCountryByIso(countries, json.address.CountryCode);
                    if (response.validation) {
                        var re = /^https?:\/\/[^/]+/;
                        window.location.href = re.exec(window.location.href)[0] + "/" + response.idCountry;
                    } else {
...
function validationCountryByIso(countries, codeCountry) {
    var answer = { validation: false };
    for (var i = 0; i < countries.length; i++) {
        if (codeCountry === countries[i].iso2) {
            answer.validation = true;
            answer.idCountry = countries[i].id;
